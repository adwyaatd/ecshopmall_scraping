...
def scrape_shop_list():
  try:
    ...
    while page_num <= max_page_num:
      elements = driver.find_elements_by_xpath("//div[@class='yuRUbf']/a")
      ...
      for url in urls:
        ...
        try:
          ...
          print(shop_name)
          shop_description = driver.find_element_by_name("description").get_attribute("content")
          contact_url = driver.find_element_by_xpath("//a[contains(@href,'/inquiry/')]").get_attribute("href")
          if is_findable_element(driver,"class_name","logoImage"):
            shop_img_url = driver.find_element_by_class_name("logoImage").get_attribute("src")
          elif is_findable_element(driver,"class_name","cot-shopLogoImage"):
            shop_img_url = driver.find_element_by_class_name("cot-shopLogoImage").get_attribute("src")
          else:
            shop_img_url = ""
          ...
          print(f"スクレイピングエラー ショップ名: {shop_name}, URL: {shop_url} 次のショップへ")
